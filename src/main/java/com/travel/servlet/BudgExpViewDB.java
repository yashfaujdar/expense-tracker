    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
      throws ServletException, IOException {
        ...
        try {
            ...
            // Finding the number of expenses in the budget
            Statement st1 = conn.createStatement();
            String sql1 = "SELECT * FROM budget WHERE trip_id='" + trip_id + "'";
            ResultSet rs1 = st1.executeQuery(sql1);

            while (rs1.next()) {
                // Generate the HTML output for each budget group
                ...
                out.println("<td><input name='eamt' class='eamt' placeholder='Amount spent'></td><input type='hidden' name='trip_id' value='" + trip_id + "'> </tr></table>");
                out.println("<div class='btn' id='add-exp-btn' onclick='add_exp(" + bid + ")'>Add new expense</div></div>");
            }
            rs1.close();
            conn.close();
        } catch (Exception e) { 
